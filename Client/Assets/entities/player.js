class Player extends Entity {
  ...
  portalSoundLogic() {
    ...
    this.portalSound.volume = lerp(this.portalSound.volume, 0.5, deltaTime / 3);
  }
  ...
  setHoldItem() {
    this.holdItem = this.inventory.items[3][this.inventory.currentSlot].item;
  }
  ...
  closeInventory() {
    ...
    if (
      ...
      switch (GetBlock(this.inventory.interactedBlock.blockType).specialType) {
        case SpecialType.SingleChest:
          playPositionalSound(this.position, "blocks/chestclosed.ogg");
          break;
      }
    }
  ...
  checkWallForPlacing() {
    ...
    let isAdjacentToBlock =
      checkAdjacentBlocks(mousePos, true) || checkAdjacentBlocks(mousePos);
  ...
  placingLogic() {
    ...
    if (
      ...
      (!this.inventory.selectedItem || !this.inventory.selectedItem.placeBlock)
    ) {
      return;
    }
  ...
  checkBlockForPlacing(block) {
    ...
    if (block.breakWithoutBlockUnderneath) {
      ...
      if (blockBeneathDef.defaultCutoff > 0 && block.onlyPlacableOn === null)
        return false;
    }
    ...
    if (block.onlyPlacableOn?.length > 0) {
      ...
      if (!block.onlyPlacableOn.includes(blockBeneath.blockType)) return false;
    }
  ...
  dropLogic() {
    ...
    if (!this.inventory.selectedBlock && this.inventory.selectedItem == null)
      return;

    if (input.isKeyDown("ShiftLeft")) {
      this.drop(this.getSelectedSlotItem(), this.getSelectedSlotItem().count);
      this.inventory.removeItem(
        3,
  ...
  breakingLogic(hover, wall = false) {
    ...
    if (!this.abilities.mayBuild || GetBlock(hover.blockType).hardness < 0) {
      this.resetBreaking();
      return;
    }
  ...
  handleSwimming() {
    ...
    const isPressingUp = input.isKeyDown("Space") || input.isKeyDown("KeyW");

    if (!this.grounded)
      this.velocity.y =
        isPressingUp && this.canMove ? -1.5 * BLOCK_SIZE : 0.8 * BLOCK_SIZE;
  }
